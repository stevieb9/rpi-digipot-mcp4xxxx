...
# HW-free unit coverage of set()/shutdown(). We mock every wiringPi call the
# module imports, so new()/set()/shutdown() run without a Pi. digitalWrite and
# spiDataRW record into a shared @events log, letting us assert the exact
# control/data bytes sent AND the CS-LOW -> SPI -> CS-HIGH framing order.

my $mod = 'RPi::DigiPot::MCP4XXXX';

my @events;

# The mock objects must be captured and kept in scope for the mocks to persist.
my $m = Mock::Sub->new;
...
like $@, qr/shutdown\(\)/, "shutdown() bad-pot message names shutdown(), not set() (F4)";
